...
void main() {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: BlocProvider<ProductBloc>.value(
          ...
          child: const AddProductPage(isAdd: true,),
        ),
      ),
    );
  }
  ...
  Future<void> _pumpUpdateWidget(WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: BlocProvider<ProductBloc>.value(
          value: mockProductBloc,
          child: const AddProductPage(isAdd: false,product: testProduct,),
        ),
      ),
    );
  }
  ...
  testWidgets('shows success Snackbar on successful product creation', (WidgetTester tester) async {
    ...
    whenListen(
      mockProductBloc,
      Stream<ProductState>.fromIterable([ProductCreatedState(product: mockProduct)]),
    );
  ...
  testWidgets('shows error Snackbar on product creation failure', (WidgetTester tester) async {
    ...
    whenListen(
      mockProductBloc,
      Stream<ProductState>.fromIterable([ProductCreatedErrorState(message: 'Failed to create product')]),
    );
    ...
    whenListen(
      mockProductBloc,
      Stream<ProductState>.fromIterable([ProductUpdatedState(product: mockProduct)]),
    );
  ...
  testWidgets('shows error Snackbar on product update failure', (WidgetTester tester) async {
    ...
    whenListen(
      mockProductBloc,
      Stream<ProductState>.fromIterable([ProductUpdatedErrorState(message: 'Failed to update product')]),
    );
